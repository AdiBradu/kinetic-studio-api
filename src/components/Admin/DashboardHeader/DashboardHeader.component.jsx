  ...
  const handleSave = async () => {
    ...
    switch (state) {
      ...
      case "comenzi":          
          ...
          await createOrder({variables: {firstName: createItem.prenume, lastName: createItem.nume, phone: createItem.telefon, email: createItem.email, region: createItem.judet, city: createItem.localitate, street: createItem.strada, streetNumber: createItem.nr, serviceId: parseFloat(createItem.serviciu) , details: dets}, refetchQueries: [ { query: GET_ALL_ORDERS }]});
      break; 
      case "admin":          
