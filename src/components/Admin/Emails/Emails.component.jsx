...
export default function Emails() {
  const [emails, setSEmails] = useState({
    id1: parseFloat(0),
    id2: parseFloat(0),
    subiect1: '',
    emailBody1: '',
  ...
  const handleEdit = async () => {
    let mails = [
      {
        id: parseFloat(emails.id1),
        email_subject: emails.subiect1,
        email_body: emails.emailBody1,
      },
      {
        id: parseFloat(emails.id2),
        email_subject: emails.subiect2,
        email_body: emails.emailBody2,
      },
    ];
    await handleEmails({
      variables: { emails: mails },
      refetchQueries: [{ query: GET_ALL_EMAILS }],
    });
  };
  ...
  return (
    <div className="emails">
      ...
      <BigSaveButton onClick={handleEdit} />
    </div>
  );
}
