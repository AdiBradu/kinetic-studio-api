...
import {
  GET_PARTNER_CURRENT_SCHEDULE,
  GET_PARTNER_FILLED_TIME_SLOTS,
...
const useGetTerapeutCalendarAndProgramari = (terapeutId) => {
  ...
  const partnerSchedQObj = useQuery(GET_PARTNER_CURRENT_SCHEDULE, {
    variables: { id: parseFloat(terapeutId.terapeut) },
  });
  const querySchedData = partnerSchedQObj?.data
    ? partnerSchedQObj.data['getPartnerCurrentSchedule']
    : [];
